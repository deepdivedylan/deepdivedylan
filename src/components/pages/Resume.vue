<template>
  <div class="lg:tw-grid lg:tw-grid-cols-4">
    <div class="lg:tw-col-span-3">
      <h1 class="tw-text-4xl tw-pb-3">{{ resume.firstname }} {{ resume.familyname }}</h1>
      <div class="lg:tw-flex tw-items-center ">
        <div class="tw-flex tw-items-center" v-for="(star, index) in resume.bio_data.stars" :key="index">
          <div class="tw-pr-3">{{ star }}</div>
          <div class="tw-pr-3" v-if="index < resume.bio_data.stars.length - 1"><StarIcon class="tw-h-3" /></div>
        </div>
      </div>
    </div>
    <ul>
      <li class="list-svg list-svg-envelope"><a :href="'mailto:' + resume.bio_data.email">{{ resume.bio_data.email }}</a></li>
      <li class="list-svg list-svg-link"><router-link to="/">{{ resume.bio_data.website }}</router-link></li>
      <li class="list-svg list-svg-linkedin"><a :href="'https://www.linkedin.com/in/' + resume.linkedin_id">{{ resume.linkedin_id }}</a></li>
      <li class="list-svg list-svg-x"><a :href="'https://www.x.com/' + resume.twitter_id">{{ resume.twitter_id }}</a></li>
      <li class="list-svg list-svg-github"><a :href="'https://www.github.com/' + resume.github_id">{{ resume.github_id }}</a></li>
    </ul>
  </div>
  <div class="tw-flex tw-items-center tw-pb-3">
    <div class="tw-pr-3">
      <UserIcon class="tw-h-6" />
    </div>
    <h2 class="tw-text-3xl">Qualifications Summary</h2>
  </div>
  <ul class="tw-list-disc tw-pb-3">
    <li v-for="(qualification, index) in resume.bio_data.summary.points" :key="index">{{ qualification }}</li>
  </ul>
  <div class="tw-flex tw-items-center tw-pb-3">
    <div class="tw-pr-3">
      <CodeBracketIcon class="tw-h-6" />
    </div>
    <h2 class="tw-text-3xl">Technical Skills</h2>
  </div>
  <ul class="tw-list-disc tw-pb-3">
    <li v-for="(skill, index) in resume.skills.details" :key="index">
      {{ skill.type }}: <Badge class="tw-mr-3" v-for="(item, innerIndex) in skill.items" :key="innerIndex">{{ item.name }}</Badge>
    </li>
  </ul>
  <div class="tw-flex tw-items-center tw-pb-3">
    <div class="tw-pr-3">
      <CommandLineIcon class="tw-h-6" />
    </div>
    <h2 class="tw-text-3xl">GitHub Projects</h2>
  </div>
  <template v-for="project in resume.github_projects.details">
    <div class="lg:tw-grid lg:tw-grid-cols-4">
      <div class="lg:tw-col-span-3">
        <a :href="'https://www.github.com/' + project.project_name">{{ project.project_name }}</a>
      </div>
      <div><em>{{ project.tagline }}</em></div>
    </div>
    <ul class="tw-list-disc tw-pb-3">
      <li v-for="(item, innerIndex) in project.description" :key="innerIndex">{{ item }}</li>
    </ul>
  </template>
  <div class="tw-flex tw-items-center tw-pb-3">
    <div class="tw-pr-3">
      <AcademicCapIcon class="tw-h-6" />
    </div>
    <h2 class="tw-text-3xl">Education</h2>
  </div>
  <table class="tw-table-auto tw-w-full tw-pb-3">
    <tr class="tw-border-b tw-border-black tw-text-left dark:tw-border-stone-200">
      <th>Degree</th>
      <th>Major</th>
      <th>Institution</th>
      <th>Graduation Year</th>
    </tr>
    <tr :class="index < resume.education.schools.length -1 ? 'tw-border-b tw-border-black dark:tw-border-stone-200' : ''" v-for="(school, index) of resume.education.schools" :key="index">
      <td>{{ school.degree }}</td>
      <td>{{ school.major }}</td>
  ...
  <div class="tw-flex tw-items-center tw-pb-3">
    <div class="tw-pr-3">
      <BriefcaseIcon class="tw-h-6" />
    </div>
    <h2 class="tw-text-3xl">Professional Experience</h2>
  </div>
  <template v-for="job in resume.experience.items">
    <h3 class="tw-flex tw-justify-between tw-w-full">
      <div>
        <em>{{ job.title }}</em>, {{ job.organisation }}, {{ job.location }}
      </div>
      <div class="tw-self-end tw-text-right">
        {{ job.from }} &ndash; {{ job.to }}
      </div>
    </h3>
    <ul class="tw-list-disc tw-pb-3">
      <li v-for="(detail, index) in job.details" :key="index">{{ detail }}</li>
      <li>Technologies Used: <Badge class="tw-mr-3" v-for="(tool, innerIndex) in job.technology_used.tools" :key="innerIndex">{{ tool.name }}</Badge></li>
    </ul>
  </template>
</template>
<script setup lang="ts">
import { AcademicCapIcon, BriefcaseIcon, CodeBracketIcon, CommandLineIcon, StarIcon, UserIcon } from '@heroicons/vue/24/solid';
import Badge from '../Badge.vue';

const resume = {
  "firstname": "Dylan",
  "familyname": "McDonald",
  "linkedin_id": "deepdivedylan",
  "github_id": "deepdivedylan",
  "twitter_id": "deepdivedylan",
  "bio_data": {
